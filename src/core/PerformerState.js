...
export class PerformerState {
     ...
     */
    constructor(colorHex, isBass = false, config) {
        this.config = config;
    ...
    updatePhysics() {
        ...
        this.current.depth = THREE.MathUtils.lerp(this.current.depth, this.target.depth, this.config.depthSmoothing);
        this.current.phaseZ = this.current.depth * this.config.grid.phaseScale;
