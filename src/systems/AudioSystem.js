...
export class AudioSystem {
    ...
    _createVoice(isBass) {
        ...
        v.osc1.frequency.value = this.config.audio.rootFreq;
        v.osc2.frequency.value = this.config.audio.rootFreq;

        v.osc1.detune.value = this.config.audio.detune;
        v.osc2.detune.value = -this.config.audio.detune;
     ...
     * @param {import('../core/PerformerState').PerformerState[]} performers - Array of performer states.
     */
    update(performers) {
        ...
        performers.forEach((p, idx) => {
            ...
            let freq = this.config.audio.rootFreq * ratio;
            ...
            const cutoff = THREE.MathUtils.lerp(this.config.audio.filterMin, this.config.audio.filterMax, height);
        ...
        pulseHz = THREE.MathUtils.clamp(pulseHz, this.config.audio.lfoRateMin, this.config.audio.lfoRateMax);
